...
class Stockissue_model extends CI_Model {
	 ...
	 public function addedit($value='')
    {
        ...
        if ($ID!='') {
            ...
            setFlashMsg('success_message',$this->MESSAGE['STOCKISSUE_UPDATED'],'alert-success');
            ...
            setFlashMsg('success_message',$this->MESSAGE['STOCKISSUE_ADDED'],'alert-success');
 ...
 public function getPartsnameByManufacturer($m_id_val='')
   ...
   public function getPartscolorByPartsId($midval='',$idval='')
    {
		...
		if($parts_id){
			...
			$this->db->group_by("part_colors");
            $this->db->order_by("part_colors",'asc');
  ...
  function getPartStockForIssue($midval='',$parts_idval='',$parts_color_val='')
	{ 
        ...
        $this->db->where("GRP.part_colors",$part_colors);
        $result=$this->db->get()->row();
    ...
    public function singleAction($value='')
    {
    	...
    	if($action!='' && $ID>0){
    		if($action=='deactive'){
    			...
    			setFlashMsg('success_message',$this->MESSAGE['STOCKISSUE_DEACTIVE_STATUS'],'alert-info');
    		}
	    else if($action=='active'){
	    	$updateData=array('status'=>1);
	    	setFlashMsg('success_message',$this->MESSAGE['STOCKISSUE_ACTIVE_STATUS'],'alert-success');
	    }
	    else if($action=='delete'){
	    	setFlashMsg('success_message',$this->MESSAGE['STOCKISSUE_DELETE'],'alert-danger');
 ...
 public function bulkAction($value='')
    {
    	...
    	if($ids && count($ids)>0){
	    	...
	    	setFlashMsg('success_message',$this->MESSAGE['STOCKISSUE_DELETE_BULK'],'alert-danger');
	    	return true;
    	}
    }
