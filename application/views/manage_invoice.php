                    ...
                    <div class="box box-primary">
                        <div class="box-header with-border">
                            ...
                            <div class="col-sm-6 text-right">
                                <?php if(checkModulePermission(16,2)){ ?>
                                    <a href="<?= $main_page ?>/addedit" class="btn btn-primary">Add <?= $heading ?></a>
                                <?php } ?>
									
                                <?php if(checkModulePermission(16,4)){ ?>
                            ...
                            <div class="table-responsive">
                                <table id="mytable" class="table table-bordered table-hover">
                                    ...
                                    <tr>
                                        ...
                                        <th>Invoice ID</th>
                                        <th>Invoice Date</th>
                                        <th>Planning Sheet ID</th>
                                        <th>Manufacturer</th>
                                        <?php if(checkModulePermission(16,5)){ ?>
                                            <th>Total Cost</th>
                                        <?php } ?>
                                        <th>Total Area</th>
                                        <th nowrap>Estimated Ship Date</th>
                                    ...
                                    if($resultList){
                                        foreach ($resultList as  $value) {
                                            ...
                                            ?>
                                            <tr <?php if(checkModulePermission(16,2)){ ?> class='clickable-row' <?php if($value->is_final==1 ){?>data-href="<?= $main_page ?>/view/<?= md5($ID) ?>" <?php } else { ?>data-href="<?= $main_page ?>/addedit/<?= md5($ID) ?>"<?php } }?> style="cursor: pointer;" >
                                                <?php if(checkModulePermission(16,4)){ ?>
                                                    <td><input type="checkbox" class="checkboxitem" value="<?= $ID ?>" name="item_ids[]"> </td>
                                                <?php } ?>
                                                <td><?= $value->invoiceid ?></td>
                                                <td><?= dateFormate($value->pi_date,"m/d/Y") ?></td>
                                                <td><?= ($value->porderid)?$value->porderid:'error' ?></td>
                                                ...
                                                <td><? 
														if($value->is_final==1)
															if($value->is_sh_complete==1)
																if($value->is_gr_complete==1)
																	echo "Received";
																else
																	echo "Shipped";
															else
