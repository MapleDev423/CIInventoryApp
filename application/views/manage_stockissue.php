              ...
              <table id="mytable" class="table table-bordered table-hover">
                ...
                if($resultList){
                  foreach ($resultList as  $value) {
                    ...
                    ?>
                      <tr <?php if(checkModulePermission(12,2)){ ?> class='clickable-row' data-href="<?= $main_page ?>/view/<?= md5($ID) ?>" <?php }?> style="cursor: pointer;">
                        <?php if(checkModulePermission(12,4)){ ?>
                        <th><input type="checkbox" class="checkboxitem" value="<?= $ID ?>" name="item_ids[]"> </th>
                        <?php } ?>
                        <td><?= dateFormate($value->issue_date,"m/d/Y") ?></td>
                        <td><?= $value->stockissue_id; ?></td>
                        <td><?= $value->employee_name; ?></td>
                        <td><?= $value->total_currentstock; ?></td>
						<td><?= $value->total_issuestock; ?></td>
                        <td nowrap class="action  t-c" style="display:none">
                        <?php if(checkModulePermission(12,2)){ ?>
                            <a href="<?= $main_page ?>/addedit/<?= md5($ID) ?>" title="Edit">
                              <i class="fa fa-pencil-square icon edit" aria-hidden="true"></i>
                            </a>
                          <?php } ?>
                          <?php if(checkModulePermission(12,3)){ ?>
                            <?php if($status_val==1){?>
